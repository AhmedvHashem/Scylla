...
fun solve(direction: String, x: Int, y: Int, width: Int, height: Int): Array<Int> {
    ...
    // return them.
    if (nextCoordinates[0] in 0 until width && nextCoordinates[1] in 0 until height &&
        (nextCoordinates[0] != x || nextCoordinates[1] != y)
    ) {
        return nextCoordinates.toTypedArray()
    } else {
        // Otherwise, return the current coordinates.
        return intArrayOf(x, y).toTypedArray()
    }
}

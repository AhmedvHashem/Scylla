...
//        println(rotateImageMatrix(mutableListOf(mutableListOf(1, 2, 3), mutableListOf(4, 5, 6), mutableListOf(7, 8, 9))))
...
fun firstDuplicate(a: MutableList<Int>): Int {
    ...
    for (i in 0 until a.size) {
        for (j in i + 1 until a.size) {
            if (a[i] == a[j] && (smallestIndex == -1 || j < smallestIndex))
                smallestIndex = j
        }
    }
...
fun firstNotRepeatingCharacter(s: String): Char {
    ...
    return dic.entries.firstOrNull {
        it.value == 0
    }?.key ?: '_'
}
...
fun asd() {
    val ma: Array<Array<Array<Array<Int>>>> = arrayOf(
        arrayOf(
            arrayOf(
                arrayOf(1, 2, 3),
                arrayOf(4, 5, 6),
